...
// 系统常量定义
const CORETHINK_VERSION = '1.0Beta';
 ...
 * @return str
 */
function getstr($str, $start, $length, $charset='utf-8', $suffix=true) {
    $str = trim($str);
    $length = $length * 2;
    if($length){
        //截断字符
...
function action_log($action = null, $model = null, $record_id = null, $user_id = null){
    ...
    if(!empty($action_info['log'])){
        if(preg_match_all('/\[(\S+?)\]/', $action_info['log'], $match)){
            $log['user']    =   get_user_info($user_id, 'nickname');
...
function get_cover($cover_id, $field = null){
    if(empty($cover_id)){
        $picture['path'] = __ROOT__.'/logo.png';
    }else{
 ...
 * @return bool
 */
function update_user_info_cache($uid){
    $list = S('sys_user_info_list');
    $list['u'.$uid] = M('Member')->find($uid);
    S('sys_user_info_list', $list);
    return true;
}
...
function sensitive_filter($text){
    $sensitive_words = explode(',', C('SENSITIVE_WORDS'));
    $sensitive_words_replace = array_combine($sensitive_words,array_fill(0,count($sensitive_words),'**'));
...
function generate_chars( $length = 8 ) { 
    ...
    for ( $i = 0; $i < $length; $i++ ) { 
        ...
        // $strs .= substr($chars, mt_rand(0, strlen($chars) – 1), 1); 
