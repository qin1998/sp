...
function get_attribute_type($type=''){
    ...
    static $_type = array(
        ...
        'date'      =>  array('日期','int(11) NOT NULL'),
        'datetime'  =>  array('时间','int(11) NOT NULL'),
        'bool'      =>  array('布尔','tinyint(1) NOT NULL'),
        ...
        'url'       =>  array('链接','int(10) UNSIGNED NOT NULL'),
        'color'     =>  array('颜色','char(7) NOT NULL'),
        'tag'       =>  array('标签','varchar(255) NOT NULL'),
    );
...
function get_child_pro_category($id){
    $map = array();
    $map['status'] = 1;
    $map['id'] = $id;
    $result = M('product_category')->where($map)->field('pid')->find();
    if ($result && is_array($result) && array_key_exists('pid', $result)) {
        if ($result['pid'] > 0) {
            $result = M('product_category')->where(array('id' => $result['pid']))->field('name')->find();
            if ($result && is_array($result) && array_key_exists('name', $result)) {
                return $result['name'];
            }
        }
    }
