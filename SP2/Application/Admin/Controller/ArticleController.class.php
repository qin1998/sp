...
class ArticleController extends AdminController {
    ...
    public function product_category(){
        ...
        $product_cate = M('product_category')->where($map)->order('sort, createtime desc')->page($p .',10')->select();
        $count = M('product_category')->count();

        $this->product_page($count, 10);
    ...
    public function order_list(){
        ...
        $product_cate = M('order')->where($map)->order('create_time desc')->page($p .',10')->select();
        $count = M('order')->count();

        $this->product_page($count, 10);
        $this->assign('cates', $product_cate);
        $this->assign('_total', count($product_cate));
        $this->meta_title = '询盘订单列表';
    ...
    public function proedit(){
        ...
        if ($data['type'] == 'deletes') {
            ...
            $this->success('删除成功', U('Article/product_category'));
        }
        ...
        if (IS_POST) {
            ...
            if ($data['type'] == 'add') {
                ...
                $this->success('新增成功', U('Article/product_category'));
            ...
            if ($data['type'] == 'delete') {
                $result = M('Document')->where(array('product_category_id' => $id))->find();
                if (!empty($result)) {
                    $this->error('can not delete ,have product in the category');
                }
                ...
                $menus = M('Product_category')->field(true)->select();
                $menus = D('Common/Tree')->toFormatTree($menus, 'name');
                $this->assign('categories', $menus);
                $this->display();
            }
        }
