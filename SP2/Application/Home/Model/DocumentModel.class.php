...
class DocumentModel extends Model {
    ...
    protected $_validate = array(
        ...
        array('title', '0,80', '标题长度不能超过80个字符', self::VALUE_VALIDATE, 'length', self::MODEL_BOTH),
    ...
    protected $_auto = array(
        ...
        array('description','getDescription', self::MODEL_BOTH, 'callback'),
     ...
     * @return array              文档列表
     */
    public function lists($category = null, $field = true, $order = "`id` desc", $status = 1, $uid = false, $pos = false, $key = null){
        $map = $this->listMap($category, $status, $uid, $pos, $key);
     ...
     * @return integer           总数
     */
    public function listCount($category, $status = 1, $uid = false, $pos = false, $key = null){
        $map = $this->listMap($category, $status, $uid, $pos, $key);
    ...
    public function prev($info){
        ...
        /* 返回前一条数据 */
        return $this->field(true)->where($map)->order('id ASC')->find();
    }
    ...
    public function update($data = null){
        ...
        //行为记录
        if($id){
            action_log('add_document', 'document', $id);
        }
     ...
     * @return array             查询条件
     */
    private function listMap($category, $status = 1, $uid = false, $pos = false, $key = null){
        /* 设置状态 */
        $map['status'] = $status;
        $map['pid'] = 0;
        if(is_numeric($uid)){
        ...
        /* 设置分类 */
        if(null != $key){
            $map['title']  = array('like', '%'.(string)$key.'%');
        }
    ...
    function getDescription(){
        if ($_POST["description"] == ""){
            $abstract = msubstr(html2text($_POST["content"]), 0,140, "utf-8", false);
        } else {
            $abstract = html2text($_POST["description"]);
        }
        return $abstract;
    }
}
